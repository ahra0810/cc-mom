 ...
 * 한국어 term + 영어 단어 (라임 태그) + 한자 (있으면, serif 태그) + 정의 박스 +
 * 시각 예시 + 관련 용어 + 영어 어원 (있으면) + 교과서 발문 예 (있으면).
 */
...
export function renderMathConceptMetaBlock(meta: MathConceptMeta, t: SetTemplate): string {
  ...
  const visualHtml = meta.visualExample
    ...
    : '';
  const originHtml = meta.englishOrigin
    ? `<div class="mc-eng-origin"><strong>어원</strong> · ${esc(meta.englishOrigin)}</div>`
    : '';
  const relatedHtml =
  ...
  if (t.metaStyle === 'festive') {
    ...
    return `<div class="meta-block meta-festive math-festive">
      ...
      ${meta.englishOrigin ? `<div class="mf-visual">📘 <strong>영어 어원</strong> · ${esc(meta.englishOrigin)}</div>` : ''}
      ${
        meta.relatedTerms && meta.relatedTerms.length
  ...
  if (t.metaStyle === 'hanja-emphasis') {
    return `<div class="meta-block meta-classic">
      ...
      ${originHtml}
      ${relatedHtml}
      ${meta.origin ? `<div class="meta-origin">${esc(meta.origin)}</div>` : ''}
    </div>`;
  }
