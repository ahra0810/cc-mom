...
export const useTestStore = create<TestStore>()(
  persist(
    (set, get) => ({
      ...
      hasUnsavedChanges: () => {
        const { currentTest, savedTests, history } = get();
        if (!currentTest) return false;
        if (history.length === 0 && currentTest.questions.length === 0) return false;
        const saved = savedTests.find((t) => t.id === currentTest.id);
        if (!saved) return currentTest.questions.length > 0;
        return JSON.stringify(saved.questions) !== JSON.stringify(currentTest.questions);
      },
    }),
    {
      name: 'quiz-maker-tests',
      partialize: (state) => ({
        savedTests: state.savedTests,
        currentTest: state.currentTest,
        initialTestSnapshot: state.initialTestSnapshot,
      }),
